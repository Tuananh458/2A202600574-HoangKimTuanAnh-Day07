    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        for piece in pieces:
            if len(piece) > self.chunk_size:
                # Mảnh còn quá lớn: đẩy buffer hiện tại ra rồi đệ quy với separator nhỏ hơn
                flush()
                chunks.extend(self._split(piece, rest))
                continue

            # Gộp mảnh nhỏ vào buffer, nhưng không vượt quá chunk_size
            tentative_len = len(separator.join(buffer + [piece]))
            if buffer and tentative_len > self.chunk_size:
                flush()
            buffer.append(piece)
